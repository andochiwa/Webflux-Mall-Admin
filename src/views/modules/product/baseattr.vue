    ...
    <el-col :span="18">
      <div class="mod-config">
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
          ...
          <el-form-item>
            <el-button @click="getDataListKey()">查询</el-button>
...
export default {
  ...
  methods: {
    ...
    async getDataList() {
      ...
      const {data} = await attr.getBaseAttrList(this.attrtype, this.id, this.pageIndex, this.pageSize)
    ...
    async getDataListKey() {
      if (this.dataForm.key === "") {
        await this.getDataList()
        return
      }
      this.dataListLoading = true;
      const {data} = await attr.getBaseAttrList(this.attrtype, this.id, this.pageIndex, this.pageSize, this.dataForm.key)
      if (data && data.data.code === 200) {
        this.dataList = data.data.list
        this.totalPage = data.data.totalCount
      } else {
    ...
    async deleteHandle(id) {
      ...
      const {data} = await attr.delete(ids)
      if (data && data.data.code === 200) {
